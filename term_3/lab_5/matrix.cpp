...
MatrixOutOfRange::MatrixOutOfRange(std::size_t row, std::size_t col)
    : _row(row), _col(col) {
...
const char *MatrixOutOfRange::what() const noexcept {
  return _message;
}
...
MATRIX_DATATYPE Matrix::operator()(size_t row, size_t col) const {
  if (row >= _rows || col >= _cols) throw MatrixOutOfRange(row, col);
